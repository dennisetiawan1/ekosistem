...
if ($search !== '') {
    $stmt = $pdo->prepare("SELECT * FROM nasabah_perorangan
        WHERE nama_perorangan LIKE ? 
        OR cif_perorangan LIKE ? 
        OR tlpn LIKE ? 
        OR 	tahun_perorangan LIKE ? 
        ORDER BY created_at DESC");
    $stmt->execute(["%$search%", "%$search%", "%$search%", "%$search%"]);
} else {
    $stmt = $pdo->query("SELECT * FROM nasabah_perorangan ORDER BY created_at DESC");
}
...
if (isset($_GET['ajax'])) {
    foreach ($nasabahList as $n) {
        ...
        echo "<tr id='perorangan_{$n['id_perorangan']}' onclick='tampilkanInfoNasabahDariServer(this.id)'>
            <td>" . htmlspecialchars($n['cif_perorangan']) . "</td>
            <td>" . htmlspecialchars($n['nama_perorangan']) . "</td>
            <td>" . htmlspecialchars($n['tlpn']) . "</td>
            <td>" . htmlspecialchars($n['tahun_perorangan']) . "</td>
            <td>" . htmlspecialchars($n['created_at']) . '<br> oleh: ' . htmlspecialchars($updatedby) . "</td>
            <td data-label='Aksi' class='kolom-btn'>
                <a class='btn-edit' href='" . $base_url . "edit/perorangan/" . $n['id_perorangan'] . "'>
                    <img src='img/edit.svg' alt='' width='16'>
                </a>
                <a class='btn-hapus' onclick='event.stopPropagation(); hapusNasabah(" . $n['id_perorangan'] . ")'>
                    <img src='img/delete.svg' alt='' width='16'>
                </a>
                <br><br>
                 <a class='btn-ekosistem' href='javascript:void(0);' onclick='event.stopPropagation(); bukaModal(" . $n['id_perorangan'] . ");'>Ekosistem</a>
            </td>
        </tr>";
    }
    exit;
...
<?php
// Ambil data nasabah
$query = $pdo->query("SELECT * FROM nasabah_perorangan ORDER BY created_at DESC");
$nasabahList = $query->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    <meta charset="UTF-8">
    <title>Daftar Nasabah Perorangan</title>
...
<body>
    ...
    <main class="right-content-informasi">
        <h1>Daftar Nasabah Perorangan</h1>
        <div method="GET" class="search-form">
            <input type="text" id="searchInput" name="q" placeholder="Cari CIF, Nama, atau No HP..." value="<?= isset($_GET['q']) ? htmlspecialchars($_GET['q']) : '' ?>">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th onclick="sortTable(2, 'telepon')">Telepon <span id="sort-telepon" class="sort-icons"><span class="asc">&#8593;</span><span class="desc">&#8595;</span></span></th>
                ...
                <tbody id="nasabahTableBody">
                    <?php foreach ($nasabahList as $n): ?>
                        <tr id="perorangan_<?= $n['id_perorangan'] ?>" onclick="tampilkanInfoNasabahDariServer(this.id)">
                            <td><?= htmlspecialchars($n['cif_perorangan']) ?></td>
                            <td><?= htmlspecialchars($n['nama_perorangan']) ?></td>
                            <td><?= htmlspecialchars($n['tlpn']) ?></td>
                            <td><?= htmlspecialchars($n['tahun_perorangan']) ?></td>
                            <td><?= htmlspecialchars($n['created_at']) ?>
                                <?php
                                $idn = $n['created_by'];
                                $updatedby = 'Pegawai';
                            ...
                            <td data-label="Aksi" class="kolom-btn">
                                <a class="btn-edit" href="<?= $base_url . 'edit/perorangan/' . $n['id_perorangan'] ?>" onclick="event.stopPropagation();">
                                    <img src="img/edit.svg" alt="" width="16">
                                </a>
                                <a class="btn-hapus" onclick="event.stopPropagation(); hapusNasabah(<?= $n['id_perorangan'] ?>)">
                                    <img src="img/delete.svg" alt="" width="16">
    ...
    <script>
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            fetch('informasi_perorangan.php?q=' + encodeURIComponent(query) + '&ajax=1')
        ...
        function bukaModal(id) {
            showLoading();
            fetch(baseUrl + 'ekosistem/' + id + '/perorangan')
                .then(response => response.json())
                .then(data => {
                    console.log(baseUrl + 'ekosistem/' + id + '/perorangan')
                    hideLoading();
                    ...
                    network.once('afterDrawing', () => {
                        const nodeId = 'perorangan_' + id; // gunakan format ID lengkap yang sesuai
                        if (nodes.get(nodeId)) {
        ...
        function tampilkanInfoNasabahDariServer(idGabungan) {
            ...
            fetch(baseUrl + 'detail/' + jenis + '/' + id)
                ...
                .then(data => {
                    ...
                    let html = '';
                    if (jenis === 'perusahaan') {
                        html = `<h3>${data.nama_perusahaan}</h3>
                ...
                `;
                        ...
                        if (data.hubungan?.length > 0) {
                            ...
                            data.hubungan.forEach(h => {
                                html += `<li>${h.nama_perusahaan} (${h.jenis_hubungan})</li>`;
                            });
                            html += `</ul>`;
                        }
                    ...
                    <h4>DPK</h4>
                    <ul>
                    <p><strong>Giro:</strong> Rp${formatRupiah(data.dpk_giro_perorangan)}</p>
                    <p><strong>Tabungan:</strong> Rp${formatRupiah(data.dpk_tabungan_perorangan)}</p>
                    <p><strong>Deposito:</strong> Rp${formatRupiah(data.dpk_deposito_perorangan)}</p>
                    <p><strong>Emerald:</strong> Rp${formatRupiah(data.dpk_emerald_perorangan)}</p>
                    </ul>
                    <hr>
                    <h4>Kredit</h4>
                    <ul>
                    <p><strong>Konsumer - Griya:</strong> Rp${formatRupiah(data.kredit_konsumer_griya__perorangan)}</p>
                    <p><strong>Konsumer - Fleksi:</strong> Rp${formatRupiah(data.kredit_konsumer_fleksi__perorangan)}</p>
                    <p><strong>CC:</strong> Rp${formatRupiah(data.kredit_cc_perorangan)}</p>
                    <p><strong>BNI INSTAN:</strong> Rp${formatRupiah(data.kredit_bni_instan)}</p>
                    </ul>
                    <hr>
                    ...
                    `;
                        ...
                        if (data.relasi?.length > 0) {
                            ...
                            data.relasi.forEach(r => {
                                html += `<li><strong>${r.nama}</strong> (${r.jenis})</li>`;
                            });
                            html += `</ul>`;
                        }
                        ...
                        if (data.hubungan_perusahaan?.length > 0) {
                            ...
                            data.hubungan_perusahaan.forEach(h => {
                                html += `<li><strong>${h.nama} (${h.jenis})</li>`;
                            });
                            html += `</ul>`;
                        }
                    }
        ...
        function tampilkanNotifikasi(pesan, warna = "#28a745") {
